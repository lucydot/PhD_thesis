...
Iodine ions are well known to form iodide chains, and the cond lengths are sensitive to charges on the iodine.
...
To allow for comparison, the 
For both the perfect and defective lattice the IPR of the acoustic
Figure \ref{defect_dispersion_IPR}c demonstrates that a large number of localised phonon modes are introduced by the defect.

The 


6.6meV=1.59THz.
Can split by atom contribution. See that I and Pb, heavier elements, are at lower energies as might be expected.
Can also locate which phonon modes have contrbutions from the two bonding iodine that form the H-centre.
Phonon mode is animated and available on youtube at.
Find that this is at a frequency of x, almost equal to the harmonic frwquency calculated for the negatively charge PES
This suggests that the approximation of a 1D PES is accuratae in this instance. 
...
electron-phonon coupling at the site is S, which corresponds to the the large region,
 % Calculate S and comment: https://hal.archives-ouvertes.fr/jpa-00213295/document

 %discuss limits - promoting and accepting modes
...
\subsection{Carrier capture mechanism and rate} \label{finalsection}

To calculate a carrier capture coefficient the electron-phonon coupling term must be calculated.
...
This is combined with the vibrational wavefunctions (see methods)
 % Plot of the schrodinger solution is in the appendix.

 At room temperature the carrier capture coefficient is
...
% The results show that neutral to negative but not the other way round. one way street
% the neutral OP will convert into negative on a short time scale via a non-radiative recombination route.
% but the negative is harmless and no charge capture is possible from it. due to large relaxation on electron capture.
% neutral not thermodynamically stable but formed from positive. 

% this conclusion is consistent with p-type doping from seebeck coefficient whihc could be attributed to negatively charge interstitials
...
\textbf{Summary}
% brief summary
	% importance of anharmoniciy
	% localised states
    % accuracy of the 1D configuration coordinate
    % one way street
