...
\end{bmatrix}
$$
This supercell expansion scales the magnitude of the lattice vectors in the $x$, $y$ and $z$ directions by a factor of $2\sqrt{2}$, $2\sqrt{2}$ and $2$ respectively. A 192-atom supercell built from a diagonal transformation matrix would increase errors due to elastic effects; \textit{e.g.} $m_t=\mathrm{diag}(1,2,8)$ would increase the elastic error along the $x$ axis, as the local relaxation extends beyond a single unit cell.
...
Lattice relaxation after electron capture leads to a large shift in the equilibrium position ($\Delta Q=$ \SI{36}{\electronvolt\per amu\tothe{1/2}\per\angstrom}).
...
The H-centre is metastable and will be formed only after photo-excitation. We have shown that it cannot be formed from hole capture at a negative charge state, but the existing literature predicts that hole capture from the positive charge state is possible. Further calculations are needed to confirm that hole capture is possible, and future work could apply the methodology outlined here to this process. 
