...
\begin{table*} \centering
\caption[Common issues that arise in the simulation of hybrid perovskites]{\label{tab:techsol} A collection of common issues that can arise in the simulation of hybrid perovskites, sourced from members of the Walsh Materials Design Group.
}
\begin{tabular}{p{4cm}p{4cm}p{6cm}}
\toprule
Technique & Symptom & Solution \\
\midrule
Geometry \newline optimisation & Partial occupancy in structure files & Test different configurations and check total energy \\
Geometry \newline optimisation & Missing H in structure files & Include H based on chemical knowledge and electron counting  \\
Geometry \newline optimisation & Slow ionic convergence & Try changing algorithm type and settings \\
...
Electronic structure & Workfunction is \newline positive & Align to external vacuum level using a non-polar surface  \\
Supercell convergence & Unusual convergence behaviour & Use only even cell expansions (e.g. $2\times2\times2$) \\
Ab initio \newline thermodynamics & No stable chemical \newline potential range & No easy fix (many hybrid\newline materials are metastable) \\
Berry phase \newline polarisation & Spontaneous polarisation is too large & Use appropriate reference \newline structure and pathway \\
Point defects & Negative formation \newline energies & Check for balanced chemical reaction and chemical potential limits \\
Point defects & Transition levels are deep in band-gap & Check supercell expansion and charged defect corrections \\
Alloyed systems & Many possible \newline configurations & Use appropriate statistical \newline mechanics \\
...
Molecular dynamics & System melts or \newline decomposes & Check $k$-points and basis set\newline convergence \\
...
Electron-phonon\newline coupling & Values far from \newline experiment & Consider anharmonic terms \newline beyond linear response theory   \\
