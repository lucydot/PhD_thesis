...
\end{equation}
where $\Psi(\{\textbf{R}_I\};\{\textbf{r}_i\})$ is the many-body wavefunction that describes the state of the system with ions $\{I\}$ in positions $\{textbf{R}_I\}$ and electrons $\{i\}$ in positions $\{\textbf{r}_i\}$.
...
\hat{H} = \frac{-\hbar^2}{2M}\nabla_{\textbf{R}}^2+\frac{-\hbar^2}{2m}\nabla_{\textbf{r}}^2+\frac{-1}{\lvert\textbf{R}-\textbf{r}\rvert},
\end{equation}
...
For a simple system, such as the particle in a box system or hydrogen atom, the Schr\"{o}dinger equation can be solved exactly for $\Psi(\textbf{r})$. Unfortunately it is not possible to solve the Schr\"{o}dinger equation exactly for more complex systems, where there are multiple electrons interacting with each other (N-body or many-body systems). 
...
The Hartree approximation assumes that the many-body wavefunction $\Psi(\{\textbf{r}_i\})^H$ is simply a product of independent single electron states $\{\phi_i(\textbf{r}_i)\}$:\autocite{Hartree1928}
\begin{equation}
\Psi^H(\{\textbf{r}_i\}) = \phi_1(\textbf{r}_1)\phi_2(\textbf{r}_2)\ldots\phi_n(\textbf{r}_n),
\end{equation}

and the system Hamiltonian becomes the sum over the Hamiltonian of each single electron:
\begin{align}
\hat{H} &= \sum_{i=1}^n\left(-\frac{\hbar^2}{2m_i}\nabla_{\textbf{r}_i}^2 - \sum_{I=1}^N\frac{e^2Z_I}{\lvert\textbf{R}_I-\textbf{r}_j\rvert} + v_i\right) \\
v_i &= \sum_{i<j}^n \int\frac{\rho_j}{\lvert\textbf{r}_i-\textbf{r}_j\rvert}dr_{ij}
rho_j &= \lvert\phi_j\rvert^2e
\end{align}

where the third term $v_i$ corresponds to the average electrostatic interaction between electron $i$ and all electrons. The approach is called the `mean-field approximation' and $v_i$ is the `Hartree potential'. A self-consistent field method (discussed further in Section \ref{SCFsubsection}) is needed to solve the Schr\"{o}dinger equation for $\Psi^H$, as the Hartree potential depends on the charge density $rho_j$, which itself depends on the single-particle wavefunction $\phi_j$.
...
Forming a many-body wavefunction $\Psi(\{\textbf{r}_i\})^{HF}$ from the Slater determinant of independent single particle electron is the Hartree-Fock approximation. To solve the Schr\"{o}dinger equation and find the ground state (lowest energy) solution for $\Psi(\{\textbf{r}_i\})^{HF}$ a variational method can be used.\autocite{Fock1930} In addition, the energy of each single particle state $\epsilon_i$ is minimised with respect to $\phi_i$ and $\{\phi_i\}$ form an orthonormal basis. Applying these principles results in the Hartree-Fock equations\autocite{Fock1930}
\begin{align}
\left[-\frac{\hbar^2}{2m_i}]\nabla_{\textbf{r}_i}^2  -\sum_{i,I}^{n,N}\frac{Z_Ie^2}{\lvert\textbf{R}_I-\textbf{r}_j\rvert} &+ v_i\right]\phi_i(\mathbf{r})+\int d\mathbf{r'}V_x(\mathbf{r},\mathbf{r'})\phi_i(\mathbf{r'}) = \epsilon_i\phi_i(\mathbf{r}), \\
rho_j &= e\sum_{i=1}^n\lvert\phi_i(\mathbf{r})\rvert^2 \\
V_x(\mathbf{r},mathbf{r'}) &= - \sum_{j=1}^n\frac{\phi_i^*(\mathbf{r'})\phi_j(\mathbf{r})}{\lvert\mathbf{r}-\mathbf{r'}\rvert}
\end{align}

$V_\mathrm{x}$ accounts for the spin exchange interaction between electrons, whereby two identical electrons (i.e. electrons with the same spin and momentum) cannot occupy the same space at the same time. This results in a repulsion between electrons with parallel spins.
