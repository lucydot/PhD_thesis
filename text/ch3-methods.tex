...
The Fermi energy is treated as a parameter, of which the defect formation energy is a linear function with a gradient equal to defect charge. This allows us to plot a graph of formation energy against Fermi energy, as shown in Figure \ref{}. Charge transition levels mark the fermi energy at which two charge states have the same defect formation energy. Electrically active defects have at least one charge transition state in the band gap. 
...
%  Electrically active defects have at least one transition level in the band gap of the material (Section \ref{defectformation}) with an associated defect wavefunction. This thesis


%Onecan then introduce the Fermi energyEFas a parameter,relative to the valence band maximum (VBM), of which eachdefect formation energy is a linear function with slopeq,inorder to elucidate likely defect properties at different dopingconditions. The standard approach to presenting such resultsis as a graph of formation energyEf(VqO) versusEF.Thermal ionizations are calculated from the formationenergies via the reactions:V+O+e−⇔V0O,V2+O+e−⇔V+O,V2+O+2e−⇔V0O.(5)The value ofEFwhich balances Eqs. (5), introduced via thee−terms, gives the transition levels from charge state to chargestate for each defect.

% - There is a linear dependance on the product of charge and fermi energy. This means that the slope of the formation energy when plotted against fermi energy gives the charge state. It also means that neutral charge states are flat. Plot as function of fermi energy. gives ionisation levels.
% ionisation levels in MAPI
...
% previous section: differences between total energies, can be calculated with DFT
%Can be as low as one part per million.
% point defects in the dilute limit when defect-defect interactions are negligible. 
% problem of periodicity (refere back to picture) - interact with itself.

% outline the supercell method - if its large enough.
