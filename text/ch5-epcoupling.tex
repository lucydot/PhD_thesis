...
\begin{equation}
g_{mnv}(\textbf{k},\textbf{q}) = -i\left(\frac{\hbar}{2N_pM_\kappa\omega_{\textbf{q}v}}\right)^{\frac{1}{2}}\ttextbf{q}\cdot\textbf{e}_{kv}(\textbf{q})V_0,
\end{equation}
...
\end{equation}
where $\kappa$ is the thermal conductivity, $\rho$ is the material density, and $C_p$ is the specific heat capacity. A derivation of the diffusion equation for a hot sphere is given in the Appendix (\ref{app:2-diffusion}).
...
\begin{figure}[h]
\includegraphics[width=\textwidth]{figures/ch5/phonondisp.png}
\caption[Harmonic phonon dispersion and eigenvectors at the $M$ and $R$ points in the Brillouin zone]{\label{ch5phonondisp} A) Harmonic phonon dispersion for MAPI. Phonon frequencies up to 5THz are considered, as wavenumbers above this correspond to the molecular vibrations of the inorganic cation.\autocite{Leguy2016} This image was generated using the sumo package.\cite{sumo2018} B) Phonon mode eigenvectors at the $M$ point in the Brillouin zone. The magnitude of the vector arrow is increased by a factor three, and only the motion of the inorganic cage is considered. This image was generated using the \textsc{ascii phonons} package available at \url{https://github.com/ajjackson/ascii-phonons}.

}
\end{figure}

We find that the bandgap $E_g(Q)$ is well described as quadratic over small $Q$, but that a bi-quadratic term is required to reproduce the correct behaviour at large $Q$ (Table \ref{RSS}). 
...
Distortions along the $M$ and $R$ phonon modes produce the familiar double-well potential energy surface shown earlier in the thesis (Figure \ref{fig2}).
...
The Frohlich polaron model has been recently recently applied to MAPI to calculate a polaron mobility from first principles.\autocite{Frost2017b} %Jarvist
...
%could include specific heat capacity stuff here
...
This is in agreement with reported experimental values from References \cite{Klein2016} (MAPI), \autocite{Zhong2017} (CdTe) and \autocite{Rosenwaks1993} (GaAs). 
...
In this chapter we have ``turned on'' temperature and introduced the phonon quasi-particle into our description of MAPI. The first consequence of this is that the electronic states couple to the normal mode vibrations of the system. I have quantified the extent of this coupling to two highly anharmonic modes at the $M$ and $R$ points in the Brillouin Zone. These modes, which are occupied at room temperature, have a large amplitude of motion and lead to a band gap broadening comparable to that found experimentally -- i.e. they play a significant role in the overall electron-phonon coupling of the system.
The second consequence on introducting temperature is that temperature variations in the bulk material lead to heat diffusion. I have shown that Fr{\:o}lich polaron theory and a classical heat diffusion model can be combined to
...
Via membership of the UK's HEC Materials Chemistry Consortium, this work used the ARCHER UK National Supercomputing Service (http://www.archer.ac.uk).
